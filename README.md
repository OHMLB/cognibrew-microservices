...
│  /api/v1/recommendation→ Recommendation Service (:8002)         │
│  /api/v1/feedback      → Feedback Service (:5001)               │
│  /api/v1/notification  → Notification Service (:8080) [SignalR] │
└────────────┬──────────────────┬────────────────────────────────┘
             │                  │
             ▼                  ▼
  ┌──────────────────┐  ┌───────────────────────────────┐
  ...
  │     (:8000)      │◄─│          (:8002)               │
  │                  │  │                               │
  │ - Menu CRUD      │  │ - Consumes face.recognized    │
  │ - Order history  │  │ - Calls Catalog for recs      │
  │ - Rec strategy   │  │ - Caches result in memory     │
  └──────────────────┘  │ - Publishes menu.recommended  │
                        └───────────────┬───────────────┘
                                        │
                                        ▼
                               ┌─────────────────┐
                             ...
                             ┌──────────┴──────────┐
                             ▼                     ▼
                    face.recognized           menu.recommended
                  (from Recognition)       (to Notification Svc)
```
